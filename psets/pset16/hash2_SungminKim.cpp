...
void rehash(Hash*& ht, int new_size) {
    ...
    list<wordcount>* new_table = new list<wordcount>[new_size];
    ht->tablesize=new_size;

    // rehashing
    for(int i=0;i<old_size;i++){
        list<wordcount> check=ht->hashtable[i];
        while(check.size()!=0){
            int index=hashfunction(ht,check.front().first);
            new_table[index].push_back(check.front());
            check.pop_front();
        }
    }
...
bool insert(Hash*& ht, string key) {
    ...
    for(it=ht->hashtable[index].begin(); it!=ht->hashtable[index].end(); it++){
        if(it->first==key) {it->second++; break;}
    }

    // if not found, make_pair(key, 1) and insert it to the table
    if(it==ht->hashtable[index].end()){
        pair<string, int> in=make_pair(key,1);
        ht->hashtable[index].push_back(in);
        ht->nelements++;
    }
    // rehash if necessary
    double loadfactor= nelements(ht)/tablesize(ht);
    if(loadfactor>=threshold(ht)) rehash(ht);
...
bool erase(Hash*& ht, string key) {
    ...
    for(it=ht->hashtable[index].begin(); it!=ht->hashtable[index].end(); it++){
        if(it->first==key){
            ...
            else if(it->second==1) ht->hashtable[index].erase(it);
...
list<wordcount> find(Hash* ht, string key) {
    ...
    if (ht->hashtable[index].size() > 0) {     // cannot use find() because of comp op
        ...
        for(it=ht->hashtable[index].begin();it!=ht->hashtable[index].end();it++){
            if(it->first==key) return ht->hashtable[index];
        }
    }
