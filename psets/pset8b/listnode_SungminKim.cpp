...
// To use STL stack, just comment out this section and include <stack> above.
#include <vector>
template <typename T>
struct stack {
	vector<T> data;
...
pNode pop_back(pNode p) {
	...
	pNode zap=p;
	pNode zap_prev=nullptr;
	while(zap->next!=nullptr){
		zap_prev=zap;
		zap=zap->next;
	}
	zap_prev->next=nullptr;
	delete zap;
...
pair<Node*, Node*> cut_in_two_halves(Node* p) {
	if(empty(p)) return make_pair(nullptr,nullptr);
	if(size(p)<2) return make_pair(p,p);
	...
	return make_pair(x, y);  // p's are just place-holders.
}
...
Node* zap_duplicates(Node* p) {
	...
	while(curr->next->next!=nullptr){
		if(curr->data==curr->next->data){
			while(true){
				if(curr->data!=curr->next->data) break;
				pNode zap=curr->next;
