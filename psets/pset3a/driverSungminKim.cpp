...
int main() {
	...
	do {
		...
		case 3:
			...
			while (1) {
				N = GetInt("\tEnter a number to compute Nth fibonacci number(0 to quit): ");
				if (N <= 0) break;
		...
		case 4:
			...
			while (1) {
				N = GetInt("\tEnter a number to compute ears of N bunnies(0 to quit): ");
				if (N <= 0) break;
		...
		case 5:
			...
			while (1) {
				N = GetInt("\tEnter a number to compute ears of N funny bunnies(0 to quit): ");
				if (N <= 0) break;
		...
		case 6:
			...
			while(1){
				N = GetInt("\tEnter a number to compute N lines of triangles(0 to quit): ");
				if (N <= 0) break;
		...
		case 7:
			...
			while(1){
				N = GetInt("\tEnter a number to compute sum of all digits for N(0 to quit): ");
				if (N <= 0) break;
		...
		case 8:
			...
			while(1){
				N = GetInt("\tEnter a number to compute numbers of 8 in N(0 to quit): ");
				if (N <= 0) break;
		...
		case 9:
			...
			pair<int, int> nlist9[] = {make_pair(2,5), make_pair(4,1), make_pair(3,3), make_pair(10,3), make_pair(4,16), make_pair(2,32)};
			for(auto x: nlist9){
			...
			while (1) {
				int x = GetInt("\tEnter base number(0 to quit): ");
				if (x <= 0) break;
				int y = GetInt("\tEnter number n to compute base to the power of n(0 to quit): ");
				if (y <= 0) break;
