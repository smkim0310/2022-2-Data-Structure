...
void shuffle(pList p) {
	...
	while(que!=nullptr){
		...
		pNode tempm=mid->next; pNode tempq=que->next;
		que->prev=mid;
		que->next=mid->next;
		mid->next->prev=que;
		mid->next=que;
...
pNode less(pList p, int z) {
	DPRINT(cout << ">find first less val than" << z << endl;);

	pNode x = begin(p);
	while(x!=end(p)&&::less(x->data,z)){
		x=x->next;
	}
	if(x==end(p)) return end(p);
...
pNode more(pList p, int z) {
	DPRINT(cout << ">find first bigger val than" << z << endl;);

	pNode x = begin(p);
	while(x!=end(p)&&more(x->data,z)){
		x=x->next;
	}
	if(x==end(p)) return end(p);
