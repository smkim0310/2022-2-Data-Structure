...
void grow(heap p, int key) {
	...
	if(full(p)) reserve(p,2*(p->capacity));
	p->nodes[++p->N]=key;
	swim(p,p->N);
...
void trim(heap p) {
	...
	if(4*(p->N)==p->capacity) reserve(p,(p->capacity)/2);
	swap(p,1,p->N--);
	sink(p,1);
